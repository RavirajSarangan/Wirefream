...
function ImageUpload() {
    ...
    useEffect(() => {
        // Check if wireframe URL is passed from generate-wireframe page
        const wireframeUrl = searchParams.get('wireframeUrl');
        if (wireframeUrl) {
            setPreviewUrl(decodeURIComponent(wireframeUrl));
            // Fetch the image and convert to file
            fetch(wireframeUrl)
                .then(res => res.blob())
                .then(blob => {
                    const file = new File([blob], 'generated-wireframe.png', { type: 'image/png' });
                    setFile(file);
                })
                .catch(err => console.error('Error loading wireframe:', err));
        }
    }, [searchParams]);
